...
const generateRandomData = (date: Date, location: number, id: number): EpaMonitorsData => {
...
export const generateHistoricalData = async () => {
    const locations = [1, 2, 3, 4, 5]; // Sample location IDs
    ...
    startDate.setFullYear(endDate.getFullYear() - 1); // 2 years ago
