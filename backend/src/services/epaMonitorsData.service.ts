...
export const fetchHistoricalEpaMonitorsDataByPeriods = async (location: number): Promise<HistoricalEpaMonitorsDataResponse> => {
    try {
        ...
        // Filter data for each time period, ensuring we only include data up to the current time
        ...
        const oneDayData = filterByDateAndTime(allHistoricalData, oneDayAgoStr);
        const oneWeekData = filterByDateAndTime(allHistoricalData, oneWeekAgoStr);
        const oneMonthData = filterByDateAndTime(allHistoricalData, oneMonthAgoStr);
        const threeMonthsData = filterByDateAndTime(allHistoricalData, threeMonthsAgoStr);
        const sixMonthsData = filterByDateAndTime(allHistoricalData, sixMonthsAgoStr);
        const oneYearData = filterByDateAndTime(allHistoricalData, oneYearAgoStr);
